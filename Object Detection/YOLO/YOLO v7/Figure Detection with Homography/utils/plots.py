...
def distance(preds, img, p):
    ...
    for o in preds :
        ...
        if class_name == "8" or "7" or "6" or "5" or "4" or "3" or "2" or "1" or "0":
            cogs.append([math.floor((o[0][0] + o[0][1]) / 2),
                        math.floor(o[0][3]),  # red point : fits quite good
                        1])
            # print(cogs)
            cv2.circle(img, (cogs[-1][0], cogs[-1][1]), 5, (255, 0, 0), -1)
        ...
        if len(data['points'])>4:
            ...
            source_points = np.array([[x1a, y1a],  # raw points
                                      [x2a, y2a],
                                      [x3a, y3a],
                                      [x4a, y4a],
                                      ])
            ...
            # plt.show()
            cv2.imwrite("warp_image.png", destination_image)
        ...
        cv2.polylines(img, np.int32([points]), False, (0, 255, 255))
        ...
        # set to zero for every frame
        A= 0


        areaA = Polygon([(x1a,y1a), (x2a,y2a),(x3a,y3a),(x4a,y4a)])  # define poligons of area

        for toppeople_cord in toppeople_cords:
            cv2.circle(img, (np.int32(toppeople_cord[0]),
        ...
        cv2.polylines(img, np.int32([points_org[:, :2]]), False, (0, 255, 255))
        cv2.putText(img, str(A), (15, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

        # show some information on the number of bounding boxes
        cv2.putText(img, ("Number of Miniature:{}".format(len(cogs))),
                    (20, 640-20),
