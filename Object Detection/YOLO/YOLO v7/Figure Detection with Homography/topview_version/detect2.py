...
from Homography_Practice.Homography_2 import get_plan_view
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if homography:
                        ...
                        # time.sleep(2)
                        # distance(pred, im0, p)
                        #cv2.setMouseCallback(str(p), select_points_src, im0)

                        # dst = cv2.imread('pan2.jpeg', -1)
                        # dst_copy = dst.copy()
                        # cv2.namedWindow('dst')
                        # cv2.moveWindow("dst", 780, 80);
                        # cv2.setMouseCallback('dst', select_points_dst,dst_copy)
            ...
            # Stream results
            if view_img:
                cv.namedWindow(str(p))
                cv2.waitKey(1)  # 1 millisecond
                src_copy = im0.copy()
                cv2.setMouseCallback(str(p), select_points_src)

                dst = cv2.imread('pan2.jpeg', -1)
                dst_copy = dst.copy()
                dst_copy = cv2.resize(dst_copy, (600,600))
                cv2.namedWindow('dst')
                cv2.moveWindow("dst", 780, 80);
                ...
                while (1):
                    ...
                    if k == ord('s'):
                        print('save points')
                        cv2.circle(im0, (src_x, src_y), 5, (0, 255, 0), -1)
                        ...
                        print('create plain view')
                        plan_view = get_plain_view(src_copy, dst_copy)
                        cv2.imshow("plain view", plan_view)
                    elif k == ord('m'):
                        print('merge views')
                        merge = merge_views(src_copy,dst_copy)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--homography', action='store_true', help='homography')
